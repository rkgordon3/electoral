...
(function($) {
  ...
  $.jCal = function (target, opt) {
    opt = $.extend({
      ...
      scrollable:        true,                   // include left/right controls for sliding calendar
  ...
  function drawCalControl (target, opt) {
    /* Attach month name and scroll controls if necessary */
    $(target).append(
      '<div class="jCal">' + 
         ( ( opt.scrollable &&  (opt.ind == 0) ) ? '<div class="left" />' : '' ) +  
          '<div class="month">' + 
            '<span class="monthYear">' + opt.day.getFullYear() + '</span>' +
            '<span class="monthName">' + opt.ml[opt.day.getMonth()] + '</span>' +
          '</div>' 
         + ( ( opt.scrollable &&  (opt.ind == ( opt.showMonths - 1 )) ) ? '<div class="right" />' : '' ) 
      + '</div>');
    
    if ( opt.monthSelect == true )
